...
class messageForwardingWebSocketServerHandler extends SimpleChannelInboundHandler<WebSocketFrame> {
        ...
        if (frame instanceof TextWebSocketFrame) { // 此处仅处理 Text Frame
            ...
            if (toChannel == null) {
                ctx.channel().writeAndFlush(new TextWebSocketFrame(
                        GSON.toJson(new ClientMessage(toUserId, fromUserId, "对方未在线" + msg))));
                //Task，存储离线消息
